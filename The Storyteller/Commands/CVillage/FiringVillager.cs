        ...
        public async Task FiringVillagerCommand(CommandContext ctx, params string[] name)
        {
            ...
            if(characterToFire != null)
            {
                if (village.Villagers.Contains(characterToFire.Id))
                {
                    ...
                    var embedKicked = dep.Embed.CreateBasicEmbed(ctx.Member, strName + " was kicked out of the village.");
                    ...
                    var embedFired = dep.Embed.CreateBasicEmbed(ctx.Member, "You have been kicked out of the village of " + village.Name);
                    await dmUser.SendMessageAsync(embed: embedFired);
                    return;
                }
            }

            var embed = dep.Embed.CreateBasicEmbed(ctx.Member, name + " does not exist or is not part of the village.");
